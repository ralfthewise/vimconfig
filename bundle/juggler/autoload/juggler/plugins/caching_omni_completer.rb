  ...
  class CachingOmniCompleter < OmniCompleter
    ...
    def generate_completions(_absolute_path, token, cursor_info)
      ...
      if @use_omni_trigger_cache
        logger.info { "Checking if we should use cache: last_linenum - #{@last_linenum}, last_matchstart - #{@last_matchstart}, last_token - #{@last_token}" }
      ...
      if use_cache_for_this_completion
        logger.info { 'Using cached omni completions' }
